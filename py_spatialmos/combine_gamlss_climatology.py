...
def combine_nwp_gamlss_climatology(
    gfse_file: Path, measurements_file: Path, target: Path
):
    ...
    with open(target, mode="w", encoding="utf-8") as f:
        f.write(
            '"yday";"kfold";"dayminute";"alt";"lon";"lat";"obs";"mean";"log_spread"\n'
        )
...
def combine_obs_gamlss_climatology(measurements_file: Path, target: Path):
    ...
    for row in measurements:
        try:
            ...
            continue
        measurements_gamlss.append(
            [
                timestamp.tm_yday,
                1,
                60 * timestamp.tm_hour,
                row[1],
                row[2],
                row[3],
                row[4],
...
def run_combine_climatology(parser_dict: Dict[str, Any]):
    ...
    logging.info(
        "GAMLSS climatology for paramaeter '%s' and observations will be created",
        parser_dict["parameter"],
    )
    measurements_file = Path(
        f"./data/get_available_data/measurements/combined/all_measurements_{parser_dict['parameter']}.csv"
    )
    target = Path(
        f"./data/spatialmos_climatology/gam/{parser_dict['parameter']}/{parser_dict['parameter']}_station_observations.csv"
    )
    ...
    for step in steps:
        logging.info(
            "GAMLSS climatology for paramaeter '%s' and step '%s' will be created",
            parser_dict["parameter"],
            f"{step:03}",
        )
        gfse_file = Path(
            f"./data/get_available_data/interpolated_station_forecasts/{parser_dict['parameter']}/GFSE_f{step:03}.csv"
        )
        target_file = Path(
            f"./data/spatialmos_climatology/gam/{parser_dict['parameter']}/climate_nwp/{parser_dict['parameter']}_{step:03}.csv"
        )
