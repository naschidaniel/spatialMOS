...
def spatial_parser_value_in_list(
    argsinfo: Dict[str, Any], key: str, options: argparse.Namespace, parsed_args
) -> Dict[str, Any]:
...
def spatial_parser(args: List[str], argsinfo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    parser.add_argument(
        "--modeltype",
        dest="modeltype",
        help=f"Enter the GFSE Mean or Spread: {argsinfo['available_modeltype']}",
        default="avg",
        type=str,
    )
    parser.add_argument(
        "--begin",
        dest="begin",
        help="Enter a number for one day in the calendar year: e.g. 1",
        default=1,
        type=int,
    )
    parser.add_argument(
        "--begindate",
        dest="begindate",
        help="Enter the begindate in the format YYYY-MM-DD.",
        default="",
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enddate",
        dest="enddate",
        help="Enter the enddate in the format YYYY-MM-DD.",
        default="",
        type=str,
    )
    parser.add_argument(
        "--folder",
        dest="folder",
        help=f"Enter a folder: {argsinfo['available_resolution']}",
        default="",
        type=str,
    )
    parser.add_argument(
        "--parameter",
        dest="parameter",
        help=f"Enter a parameter from the list: {argsinfo['available_parameter']}",
        default="",
        type=str,
    )
    parser.add_argument(
        "--resolution",
        dest="resolution",
        help=f"Model initialization hour: {argsinfo['available_resolution']}",
        default=0.5,
        type=float,
    )
    parser.add_argument(
        "--script",
        dest="script",
        help=f"Enter a script from the list: {argsinfo['available_script']}",
        default="",
        type=str,
    )
