...
def convert_measurements(
    measurements: Dict[str, Dict[str, Union[str, float]]], columns: list[str]
):
...
def gribfiles_to_json(file_avg, file_spr, parameter, subset):
    ...
    with xr.open_dataset(str(file_avg), engine="cfgrib") as ds_avg, xr.open_dataset(
        str(file_spr), engine="cfgrib"
    ) as ds_spr:
        latitude = np.arange(
            subset["S"], subset["N"] + subset["resolution"], subset["resolution"]
        )
        longitude = np.arange(
            subset["W"], subset["E"] + subset["resolution"], subset["resolution"]
        )
        ...
        if parameter not in [
            ...
            raise ValueError("The parameter '%s' cannot be processed." % parameter)
        ...
        data = spatial_rust_util.combine_gribdata(
            list(latitude), list(longitude), values_avg, values_spr
        )
        return {
            "parameter": parameter,
            "anal_date": ds_avg.time.dt.strftime("%Y-%m-%d %H:%M:%S").values.tolist(),
            "valid_date": ds_avg.valid_time.dt.strftime(
                "%Y-%m-%d %H:%M:%S"
            ).values.tolist(),
            "yday": ds_avg.valid_time.dt.dayofyear.values.tolist(),
            "dayminute": int(ds_avg.valid_time.dt.hour.values)
            * 60,  # * ds_avg.valid_time.dt.minute.values),
