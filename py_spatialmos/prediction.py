...
def run_spatial_predictions(parser_dict):
    ...
    for json_file in json_files:
        ...
        if not os.path.exists(climate_spatialmos_file) or not os.path.exists(climate_spatialmos_nwp_file) or not os.path.exists(spatialmos_coef_file):
            logging.warning("For the parameter '%s', step '%03d' and yday '%03d' the required climatologies or coefficients are missing.",
                            parser_dict["parameter"], gribfiles_data["step"], gribfiles_data['yday'])
            ...
            spatialmos_run_status.append(
                {"status": "failed", "step": f"{gribfiles_data['step']:03d}"})
            continue
...
def spatial_prediction(alt_file, alt_area_file, climate_spatialmos_file, climate_spatialmos_nwp_file, data_path_spool, gribfiles_data, spatial_alt_area_file, spatialmos_coef_file, spatialmos_run_status, parser_dict, gadm36_shape_file):
    ...
    xx_spatialmos, yy_spatialmos = np.meshgrid(
        lons_spatialmos, lats_spatialmos)
    ...
    mean_interpolation = griddata(
        nwp_df[["longitude", "latitude"]], nwp_df["mean"], (xx_spatialmos, yy_spatialmos), method="linear")
    log_spread_interpolation = griddata(
        nwp_df[["longitude", "latitude"]], nwp_df["log_spread"], (xx_spatialmos, yy_spatialmos), method="linear")
    ...
    climate_spatialmos = pd.read_csv(
        climate_spatialmos_file, header=0, index_col=0)
    ...
    spatialmos = pd.concat(
        [spatial_alt_area, climate_spatialmos], axis=1, sort=True)
    spatialmos = pd.concat(
        [spatialmos, climate_spatialmos_nwp], axis=1, sort=True)
    ...
    log_spread_fit = spatial_plots.reshapearea(
        spatialmos["log_spread_fit"], alt)
    ...
    log_spread_nwp_anom = (
        log_spread_interpolation_spatial_area.data - log_spread_fit) / log_spread_sd

    # Generate spatialmos spatial predictions
    spatialmos_coef = spatialmos_coef.apply(pd.to_numeric)
    spatialmos_anom = spatialmos_coef["intercept"][0] + \
        spatialmos_coef["mean_anom"][0] * nwp_anom
    spatialmos_mean = spatialmos_anom * climate_sd + climate_fit
    spatialmos_log_anom_spread = spatialmos_coef["intercept_log_spread"][0] + \
    ...
    spatialmos_point = pd.DataFrame({"lat": yy_spatialmos.flatten().tolist(), "lon": xx_spatialmos.flatten().tolist(
    ), "spatialmos_mean": spatialmos_mean.flatten().tolist(), "spatialmos_spread": spatialmos_spread.flatten().tolist()})
    ...
    with open(data_path_spool.joinpath(filename_spatialmos_step), "w") as f:
