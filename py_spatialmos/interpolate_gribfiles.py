...
def run_interpolate_gribfiles(parser_dict: Dict[str, Any]):
    ...
    for provider in ["lwd_stations", "suedtirol_stations", "zamg_stations"]:
        station_files = pathlib.Path(
            "./data/get_available_data/measurements/combined/"
        ).glob(f"{provider}_{parser_dict['parameter']}*.csv")
...
def interpolate_gribfiles(
    gribdata: Dict[str, Any],
    csv_writer: SpatialWriter,
    station_locations: list[list[float]],
):
