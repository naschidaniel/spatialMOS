...
class SuedtirolData:
    ...
    def request_data(cls, request_type: str, url="") -> dict:
        ...
        if request_type == "stations":
            return {
                station["properties"]["SCODE"]: station["properties"]
                for station in data_dict["features"]
            }
        return data_dict


def suedtirol_spatial_converter(
    measurements: Dict[str, Dict[str, Union[str, float]]], filename: Path
) -> None:
    ...
    try:
        ...
        if len(measurements_write_lines) != 0:
            with open(filename, mode="w", newline="", encoding="utf-8") as target:
                ...
                logging.info(
                    "%s data lines will be written for the station %s.",
                    len(measurements_write_lines),
                    station,
                )
...
def fetch_suedtirol_data(begindate: str, enddate: str, data_path: Path):
    ...
    for station in stations.values():
        ...
        if len(list(measurements.keys())) == 0:
            logging.info(
                "No data relevant for spatialMOS are available for the station %s.",
                station["SCODE"],
            )
            continue
