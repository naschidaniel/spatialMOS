...
def get_file_names(
    data_path_gribfile: pathlib.Path, baseurl, date, mem, step, modeltype, resolution
):
    ...
    if modeltype in ["avg", "spr"]:
        if resolution == 0.5:
            ...
            filename = (
                f'ge{modeltype}.t{date.strftime("%H")}z.pgrb2af{step:02d}'
                if step < 100
                else f"{step:03d}"
            )
        else:
        ...
        gribfile_path = (
            f"{date.strftime('GFSE_%Y%m%d_%H00')}_{modeltype}_f{step:03d}.grb2"
        )
        ...
        gribfile = os.path.join(
            date.strftime(baseurl),
            f"ge{'c' if mem == 0 else 'p'}{mem:02d}.t{date.strftime('%H')}z.pgrb2a.0p50.f{step:03d}",
        )
...
def parse_index_file(idxfile, params):
    ...
    for line in data:
        ...
        if not match:
            logging.error("whoops, pattern mismatch '%s'", line)
            raise RuntimeError
...
def download_grib(grib, gribfile_path, required):
    ...
    try:
        ...
        logging.error(
            "The Grbifile '%s' could not be loaded and saved in '%s'",
            grib,
            gribfile_path,
        )
        logging.exception(ex)
...
def fetch_gefs_data(modeltype, date, parameter, resolution):
    ...
    if modeltype in ["avg", "spr"]:
        ...
        if resolution == 1:
            data_path = pathlib.Path(
                f"./data/get_available_data/gefs_avgspr_forecast_p1/{parameter}"
            )
            ...
            data_path = pathlib.Path(
                f"./data/get_available_data/gefs_avgspr_forecast_p05/{parameter}"
            )
        ...
        data_path = pathlib.Path(
            f"./data/get_available_data/gefs_ens_forecast_p05/{parameter}"
        )
        members = range(0, 30 + 1, 1)
        logging.info(
            "Downloading members: {%s}", ", ".join([f"{x:d}" for x in members])
        )
        # url exchanged on 2020-09-23 response with a 404 error
    ...
    logging.info(
        "For date/model initialization %s", date.strftime("%Y-%m-%d %H:%M UTC")
    )
    ...
    for mem in members:
        ...
        for step in steps:
            ...
            if modeltype in ["avg", "spr"]:
                ...
                logging.info(
                    "Processing +%sh forecast, member %s", f"{step:03d}", f"{mem:02d}"
                )
            ...
            if os.path.isfile(files["gribfile_path"]) or os.path.isfile(
                files["jsonfile_path"]
            ):
                logging.info(
                    "gribfile_path file exists, skip: %s", files["gribfile_path"]
                )
                logging.info(
                    "-------------------------------------------------------------------------------------------------"
                )
