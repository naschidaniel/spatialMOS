            ...
            for entry in json_data["SpatialMosPoint"]:
                ...
                if lat not in predictions.keys():
                    predictions.setdefault(
                        ...
                        {
                            entry["lon"]: {
                                "spatialmos_mean": [entry["spatialmos_mean"]],
                                "spatialmos_spread": [entry["spatialmos_spread"]],
                            }
                        },
                    )
                    continue
                if lon not in predictions[lat].keys():
                    ...
                    predictions[lat][lon]["spatialmos_mean"].append(
                        entry["spatialmos_mean"]
                    )
                    predictions[lat][lon]["spatialmos_spread"].append(
                        entry["spatialmos_spread"]
                    )
...
def run_combine_predictions(parser_dict):
    ...
    logging.info(
        "The steps '%s' in the folder '%s' for '%s' and date '%s' will combined",
        steps,
        data_path_media,
