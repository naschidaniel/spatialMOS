...
def run_data_for_spatialmos():
    ...
    for provider in ["lwd", "suedtirol", "zamg"]:
        ...
        for parameter in ["tmp_2m", "rh_2m"]:
            logging.info(
                "'%s' Measurements for the parameter '%s' will be combined",
                provider,
                parameter,
            )
            data = []
            for measurements_file in Path(
                "./data/get_available_data/measurements"
            ).glob(f"{provider}_measurements_all_*.csv"):
                ...
                if len(data_new) <= 3:
                    logging.warning(
                        "There is no data so the csv file '%s' will be skiped.",
                        measurements_file,
                    )
                    continue
            ...
            targetfile_parameter = Path(target_path).joinpath(
                f"{provider}_measurements_{parameter}_{start_date}_{end_date}.csv"
            )
            targetfile_stations = Path(target_path).joinpath(
                f"{provider}_stations_{parameter}_{start_date}_{end_date}.csv"
            )
            with open(
                targetfile_parameter, mode="w", newline="", encoding="utf-8"
            ) as target_parameter, open(
                targetfile_stations, mode="w", newline="", encoding="utf-8"
            ) as target_stations:
                data_for_spatialmos(
                    data, parameters, parameter, target_parameter, target_stations
                )
...
def select_parameters(folder: str) -> Dict[str, Dict[str, str]]:
    ...
    if folder == "lwd":
        ...
        raise RuntimeError(
            f"The run_combine_data is for the folder '{folder}' is not implemented"
        )
    return parameters


def data_for_spatialmos(
    data: List[List[str]],
    parameters: Dict[str, Dict[str, str]],
    parameter: str,
    target_parameter: TextIO,
    target_stations: TextIO,
):
    ...
    spatialmos_subset: Dict[str, float] = {"W": 10.0, "E": 12.0, "S": 46.0, "N": 48.0}
    ...
    for row in data:
        ...
        try:
            ...
            logging.error(
                "Could not convert string %s, %s to float",
                row[lon_index],
                row[lat_index],
            )
            continue
        ...
        if lat > 47.0 and float(row[alt_index]) <= 400:
            logging.error(
                "The altitude '%s' for the point '%s, %s' is probably wrong",
                row[alt_index],
                row[lon_index],
                row[lat_index],
            )
        ...
        try:
            if parameter == "tmp_2m":
                ...
                if not -45 <= value <= +45:
                    logging.warning(
                        "tmp_2m value '%s' '%s' out of range", row[0], value
                    )
                    continue
            if parameter == "rh_2m":
        ...
        try:
            writer_parameter.append(
                [row[date_index], row[alt_index], str(lon), str(lat), value]
            )
    ...
    for parameter in ["rh_2m", "tmp_2m"]:
        ...
        for provider in ["lwd", "suedtirol", "zamg"]:
            measurements_files = Path(
                "./data/get_available_data/measurements/combined/"
            ).glob(f"{provider}_measurements_{parameter}_*.csv")
            for measurements_file in measurements_files:
                logging.info("Reading %s", measurements_file)
                with open(
                    measurements_file, mode="r", newline="", encoding="utf-8"
                ) as f:
                    data_new = list(csv.reader(f, delimiter=";"))
                    header = data_new[0:2]
                    data = data + data_new[2:]
        ...
        data = sorted(data[2:], key=lambda x: x[0])
        data = header + data
        measurements_file_all = Path(
            f"./data/get_available_data/measurements/combined/all_measurements_{parameter}.csv"
        )
