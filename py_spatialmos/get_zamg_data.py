...
class ZamgData:
    ...
    def get_station_locations() -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(
                "The loaded csv Data from the '%s' could not be converted into a csv file.",
                url,
            )
            raise ex
...
class ZamgSpatialConverter:
    ...
    def __init__(self, target: TextIO):
        ...
        max_retries = 3
        now_hour = int(
            datetime.datetime.now(pytz.timezone("Europe/Vienna")).strftime("%H")
        )
        while retry <= max_retries:
            for state in federal_state:
                ...
                if now_hour == int(raw_text_time):
                    for row in measurements_optimized:
                        ...
                        logging.info(
                            "The key '%s' is searched in the station_info", key
                        )
                        try:
                            ...
                            logging.warning(
                                "No data could be found for the key '%s'", key
                            )
                            row.insert(3, "-999")
                            row.insert(4, "-999")
            ...
            logging.error(
                "No current data could be loaded for the federal state %s and retry %s/%s ",
                federal_state,
                retry,
                max_retries,
            )
            ...
            raise RuntimeError(
                f"The maximum number of retries was reached {retry}/{max_retries} and not all data could be saved."
            )
    ...
    def manipulate_html_text(raw_html_text: str) -> Tuple[List[List[str]], str]:
        ...
        raw_text_date_begin = raw_html_text.find('<h1 id="dynPageHead">') + len(
            '<h1 id="dynPageHead">'
        )
        ...
        raw_text_time_begin = raw_html_text.find(
            "Aktuelle Messwerte der Wetterstationen von "
        ) + len("Aktuelle Messwerte der Wetterstationen von ")
        ...
        raw_text_measurements = raw_html_text[
            raw_text_measurements_begin:raw_text_measurements_end
        ].split("\n")

        # Removing the HTML tag
        html_tag_regex = re.compile(r".*?\>(.*?)\<")
        measurements = [
            re.findall(html_tag_regex, line) for line in raw_text_measurements
        ]
        measurements = [list(filter(None, stations)) for stations in measurements]
        measurements = [
            list(
                map(
                    lambda x: x.strip(" ").replace("Windstille", "Windstille, 0"),
                    stations,
                )
            )
...
def fetch_zamg_data(data_path: Path):
    ...
    with open(
        data_path.joinpath(f"zamg_{utcnow_str}.csv"), "w", newline="", encoding="utf-8"
    ) as target:
