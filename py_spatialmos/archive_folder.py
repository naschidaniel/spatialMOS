...
def archive_folder(archive_path, data_path):
    ...
    tarfile = os.path.join(
        archive_path,
        f"{foldername.name}_{dt.utcnow().strftime('%Y-%m-%d_%H_%M_%S')}.tar.gz",
    )
    logging.info(
        "%s files were compressed with tar into the file '%s'.",
        len(list(Path(data_path).glob("*"))),
        tarfile,
    )
...
def untar_archive_files(archive_path: Path, data_path: Path, file_prefix):
    ...
    for tarfile in archive_path.glob(f"{file_prefix}*.tar.gz"):
        ...
        if system_run != 0:
            raise RuntimeError(
                f"The archive file '{archive_path}' could not be unpacked."
            )
