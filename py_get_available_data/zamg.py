    ...
    while now_hour != raw_text_time:
        if retry <= 3:
            ...
            for state in federal_state:
                ...
                df_stat, raw_text_time = download_zamg_page_data(data_path, state, "stat")
                # Station values and current pressure at sea level
                df_red, raw_text_time = download_zamg_page_data(data_path, state, "red")
                ...
                logging.info("{:18} | Uhrzeit Text {} | Zeit {}".format(state, raw_text_time, now_hour))
                time.sleep(10)

            # Check whether all federal states have been successfully loaded
            if file_counter != 9:
                logging.error("Not all federal states could be downloaded successfully. The process is repeated. | Retry {} ".format(retry))
                retry = retry + 1
                file_counter = 0
                # Waiting for next retry
                time.sleep(600)
                logging.info("Download -------------------------------------------- Retry {} --------------------------------------------".format(retry))
            else:
                logging.info("---------------------- ZAMG Website -----------------------------------")

        else:
            logging.error("nicht alle Files downgeloadet | Retryversuch {}".format(retry))
            sys.exit(1)
...
def download_zamg_page_data(data_path, state, pressure):
    ...
    if req_page.status_code == 200:
        ...
        raw_text = raw_text.replace('<small style="font-size:0.85em;">m</small>', '')
        raw_text = raw_text.replace('<small style="font-size:0.85em;">mm</small>', '')
        raw_text = raw_text.replace('<small style="font-size:0.85em;">hPa</small>', '')
        raw_text = raw_text.replace(
        ...
        raw_text_date_begin = re.search('<h1 id="dynPageHead">', raw_text).end()
        ...
        measured_values_end = re.search('Die Messwerte in dieser Liste', raw_text).start()
        ...
        ld_name = "ld" + pressure
        df = pd.DataFrame(text_filter, columns=["station", "alt", "t", "rf", "wg", "wsg", "regen", "sonne", ld_name])
        df = df.dropna()
        ...
        logging.error(f"The request for the URL '{url_page}' returned the status code 404")
        df = None
        raw_text_time = None
        return(df, raw_text_time)
