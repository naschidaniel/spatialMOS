    ...
    try:
        ...
        if PARSER_DICT["script"] == "get_gefs_forecasts":
            ...
            get_gefs_forecasts.fetch_gefs_data(
                PARSER_DICT["modeltype"],
                PARSER_DICT["date"],
                PARSER_DICT["parameter"],
                PARSER_DICT["resolution"],
            )
            ...
            PARSER_DICT = spatial_parser.spatial_parser(arguments, argsinfo)
            logging.info(
                "The data suedtirol download from '%s' to '%s' has started.",
                PARSER_DICT["begindate"],
                PARSER_DICT["enddate"],
            )
            ...
            raise RuntimeError(
                f"The script '{PARSER_DICT['script']}' has not yet been implemented."
            )

        DURATION = datetime.datetime.now() - STARTTIME
        logging.info(
            "The script '%s' has run successfully in %s",
            PARSER_DICT["script"],
            DURATION,
        )
