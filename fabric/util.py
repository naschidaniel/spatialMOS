...
def check_upstream(c):
    """Check main"""
    print(
        "Should the diff between origin/main and the current branch be checked? [yes/no]"
    )
    answer = input()
    ...
    if answer.upper() in ("N", "NEIN", "NO"):
        logging.warning(
            "The dff between origin/main and current branch is not checked."
        )
    else:
        if c.run("git rev-parse --abbrev-ref HEAD", hide=True).stdout.strip() != "main":
            raise RuntimeError(
                "You are not in the main branch. Only the main branch can be uploaded onto the server."
            )

        c.run("git fetch origin main", hide=True)
        if c.run("git diff origin/main", hide=True).stdout.strip() != "":
            raise RuntimeError(
                "Your local branch differs from upstream main (run git diff)"
            )
...
def write_statusfile_and_success_logging(taskname):
    ...
    status = {
        ...
        "taskMaxAgeTime": (datetime.now() + timedelta(minutes=max_age)).strftime(
            "%Y-%m-%dT%H:%M:%S"
        ),
        "maxAge": max_age,
    }
