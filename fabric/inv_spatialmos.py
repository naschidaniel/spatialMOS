...
def archive_folder__gefs_avgspr_forecast_p05(c):
    ...
    util.write_statusfile_and_success_logging(
        archive_folder__gefs_avgspr_forecast_p05.__name__
    )
...
def pre_processing_gribfiles__ugrd_10m(c):
    ...
    util.write_statusfile_and_success_logging(
        pre_processing_gribfiles__ugrd_10m.__name__
    )
...
def pre_processing_gribfiles__vgrd_10m(c):
    ...
    util.write_statusfile_and_success_logging(
        pre_processing_gribfiles__vgrd_10m.__name__
    )
...
def pre_processing_gamlss_crch_climatologies(c, parameter):
    ...
    cmd = [
        "python",
        "./spatialmos/pre_processing_gamlss_crch_climatologies.py",
        "--parameter",
        parameter,
    ]
    inv_docker.run_py_container(c, cmd)
...
def init_topography(c):
    ...
    if not os.path.exists("./data/get_available_data/gadm/gadm36_AUT_shp"):
        req_gadm36_zip_file = (
            "https://biogeo.ucdavis.edu/data/gadm3.6/shp/gadm36_AUT_shp.zip"
        )
        ...
        if req_shapefile.status_code == 200:
            ...
            c.run(
                f"unzip {gadm36_zip_file} -d ./data/get_available_data/gadm/gadm36_AUT_shp"
            )
        else:
            raise RuntimeError(
                f"There was a problem with the download of the shapefile '{req_gadm36_zip_file}'"
            )
...
def merge_statusfiles(c):  # pylint: disable=W0613
    ...
    for file in sorted(statusfiles_path.glob("*.json")):
        ...
        with (open(file, mode="r", encoding="UTF-8")) as f:
            status = json.load(f)
            status["failed"] = datetime.now() > datetime.strptime(
                status["taskMaxAgeTime"], "%Y-%m-%dT%H:%M:%S"
            )
        statusfiles.append(status)

    settings = util.read_settings()
    systemchecks_done = sorted([c["taskName"] for c in statusfiles])
    systemchecks_available = [
        check
        for check in sorted(settings["systemChecks"].keys())
        if check != merge_statusfiles.__name__
    ]
    systemchecks_missing = [
        check for check in systemchecks_available if check not in systemchecks_done
    ]
    ...
    status = {
        ...
        "taskMaxAgeTime": (
            datetime.now()
            + timedelta(
                minutes=int(settings["systemChecks"][merge_statusfiles.__name__])
            )
...
def r_gamlss_crch_model(c, validation, parameter):
    ...
    cmd = [
        "Rscript",
        "./r_spatialmos/gamlss_crch_model.R",
        "--validation",
        validation,
        "--parameter",
        parameter,
...
def r_spatial_climatologies_nwp(c, begin, end, parameter):
    ...
    cmd = [
        "Rscript",
        "./r_spatialmos/spatial_climatologies_nwp.R",
        "--begin",
        begin,
        "--end",
        end,
...
def r_spatial_climatologies_obs(c, begin, end, parameter):
    ...
    cmd = [
        "Rscript",
        "./r_spatialmos/spatial_climatologies_observations.R",
        "--begin",
        begin,
        "--end",
        end,
...
def remove_old_climatologies(c):  # pylint: disable=W0613
    ...
    folders = [
        "./data/spatialmos_climatology/gam/rh_2m/climate_spatialmos",
