...
def str2bool(string):
    ...
    if string not in ["True", "False"]:
        raise ValueError(
            "Check the typo (True || False) of ignore-existing in the settings file."
        )
...
def exclude_include_ignore(settings, rsync_direction, rsync_task):
    ...
    if "ignore-existing" in settings[rsync_direction][rsync_task]:
        ignore_existing = str2bool(
            settings[rsync_direction][rsync_task]["ignore-existing"]
        )
    else:
        ignore_existing = False

    logging.info(
        "The settings %s from the settings.json file are used for the production.",
        rsync_task,
    )
...
def scp_push(remote_user, remote_host, source_file, destination_file):
    ...
    scp_cmd = [
        "scp",
        f"{source_file}",
        f"{remote_user}@{remote_host}:{destination_file}",
    ]
...
def scp_get(remote_user, remote_host, source_file, destination_file):
    ...
    scp_cmd = [
        "scp",
        f"{remote_user}@{remote_host}:{source_file}",
        f"{destination_file}",
    ]
    logging.info("The following command was executed with scp: %s", scp_cmd)
    subprocess.run(scp_cmd, check=True)
...
def push(c, what):  # pylint: disable=W0613
    ...
    if what not in ["sourcefiles", "climatologies", "staticfiles"]:
        raise RuntimeError(
            f"Your entry '{what}' was incorrect. Please read the Readme.md"
        )

    rsync_direction = "rsync_push"
    include, exclude, exclude_from, ignore_existing = exclude_include_ignore(
        settings, rsync_direction, what
    )
    rsync_push(
...
def get(c):  # pylint: disable=W0613
    ...
    for rsync_task in settings[rsync_direction]:
        print(rsync_task)
        include, exclude, exclude_from, ignore_existing = exclude_include_ignore(
            settings, rsync_direction, rsync_task
        )
        rsync_get(
            settings["REMOTE_USER"],
            settings["REMOTE_HOST"],
