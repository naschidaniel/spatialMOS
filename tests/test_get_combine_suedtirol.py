...
with open(
    Path(
        "./tests/testdata/test_get_combine_suedtirol/suedtirol_20210101_20210102_2021-05-13T19_00_00.csv"
    ),
    "r",
    encoding="utf-8",
) as testdata:
...
with open(
    Path(
        "./tests/testdata/test_get_combine_suedtirol/suedtirol_20210101_20210102_2021-05-13T19_00_00_rh_2m.csv"
    ),
    "r",
    encoding="utf-8",
) as testdata:
...
with open(
    Path(
        "./tests/testdata/test_get_combine_suedtirol/suedtirol_20210101_20210102_2021-05-13T19_00_00_rh_2m_stations.csv"
    ),
    "r",
    encoding="utf-8",
) as testdata:
...
class TestExitCodes(unittest.TestCase):
    ...
    def test_combine_data_suedtirol(self):
        ...
        get_suedtirol_data.fetch_suedtirol_data(
            "20210101", "20210102", csv_files_temp_path
        )
        # All measurements
        temp_measurements_file = create_csv_tempfile()
        with open(
            Path(temp_measurements_file), mode="w", newline="", encoding="utf-8"
        ) as target:
    ...
    def test_data_for_spatialmos(self):
        ...
        with open(
            temp_parameter_file, mode="w", newline="", encoding="ISO-8859-1"
        ) as target_parameter, open(
            temp_stations_file, mode="w", newline="", encoding="ISO-8859-1"
        ) as target_stations:
            combine_measurements.data_for_spatialmos(
                MEASUREMENTS_OK, PARAMETERS, "rh_2m", target_parameter, target_stations
            )
