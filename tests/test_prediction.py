...
class TestRustModules(unittest.TestCase):
    ...
    def test_spatial_prediction(self):
        ...
        with open(
            Path("./tests/testdata/test_prediction/GFSE_20210803_0000_f006.json"),
            "r",
            encoding="utf-8",
        ) as f:
        ...
        climate_spatialmos_file = Path(
            "./tests/testdata/test_prediction/yday_215_dayminute_360.csv"
        )
        climate_spatialmos_nwp_file = Path(
            "./tests/testdata/test_prediction/yday_215_dayminute_360_step_006.csv"
        )
        spatialmos_coef_file = Path(
            "./tests/testdata/test_prediction/spatialmos_coef_tmp_2m_006.csv"
        )
        spatial_alt_area_file = Path(
            "./tests/testdata/test_prediction/spatial_alt_area_df.csv"
        )
        gadm36_shape_file = Path(
            "./tests/testdata/test_prediction/gadm36_AUT_shp/gadm36_AUT_0.shp"
        )
        ...
        prediction.write_spatialmos_run_file(
            data_path_spool, spatialmos_run_status, parser_dict["parameter"]
        )

        with open(
            Path("./tests/testdata/test_prediction/spatialmosrun_tmp_2m.json"),
            "r",
            encoding="utf-8",
        ) as f_ok, open(
            data_path_spool.joinpath("spatialmosrun_tmp_2m.json"), "r", encoding="utf-8"
        ) as f:
        ...
        with open(
            Path("./tests/testdata/test_prediction/20210803_step_006.json"),
            "r",
            encoding="utf-8",
        ) as f_ok, open(
            data_path_spool.joinpath("20210803_step_006.json"), "r", encoding="utf-8"
        ) as f:
