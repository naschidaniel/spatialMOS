...
test_avg_gribfile = (
    Path(__file__)
    .parent.absolute()
    .joinpath("./testdata/test_spatial_util/GFSE_20210416_0000_avg_f006.grb2")
)
test_spr_gribfile = (
    Path(__file__)
    .parent.absolute()
    .joinpath("./testdata/test_spatial_util/GFSE_20210416_0000_spr_f006.grb2")
)
...
test_info_file = {
    ...
    "resolution": 0.5,
    "values_avg": [
        [2.44, 2.92, 3.47, 3.2, 3.18],
        [-0.09, 2.21, 2.43, 2.89, 3.0],
        [-1.42, 1.83, 2.53, 2.74, 2.79],
        [-3.0, -1.68, 0.48, 2.5, 3.27],
        [-0.57, -0.54, 1.07, 2.83, 2.89],
    ],
    "values_spr": [
        [0.14, 0.14, 0.11, 0.13, 0.17],
        [0.23, 0.14, 0.13, 0.14, 0.12],
        [0.11, 0.09, 0.13, 0.13, 0.16],
        [0.16, 0.16, 0.15, 0.13, 0.17],
        [0.18, 0.17, 0.16, 0.14, 0.19],
    ],
...
class TestSpatialUtil(unittest.TestCase):
    ...
    def test_spatial_parser_ok(self):
        ...
        parsed_args = spatial_parser.spatial_parser(TEST_ARGS_OK, TEST_ARGSINFO)
