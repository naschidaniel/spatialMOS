...
class TestRustModules(unittest.TestCase):
    ...
    def test_get_value_from_gribdata(self):
        ...
        self.assertEqual(
            4.3,
            spatial_rust_util.get_value_from_gribdata(
                latitudes, longitudes, values_avg, 8.5, 45.0
            ),
        )
        self.assertEqual(
            -0.41,
            spatial_rust_util.get_value_from_gribdata(
                latitudes, longitudes, values_avg, 9.0, 46.0
            ),
        )
        ...
        self.assertEqual(
            0.87,
            spatial_rust_util.get_value_from_gribdata(
                latitudes, longitudes, values_spr, 8.0, 46.0
            ),
        )
        self.assertEqual(
            0.32,
            spatial_rust_util.get_value_from_gribdata(
                latitudes, longitudes, values_spr, 9.0, 45.0
            ),
        )
